...
int main()
{
	...
	// uint8_t datas[3*3] = { 5,3,1,4,6,4,8,4,3 };
	...
	{
		counts[datas[i]]++;
		if (_max_count_val < counts[datas[i]])
			_max_count_val = counts[datas[i]];
		if (_min_count_val > counts[datas[i]])
			_min_count_val = counts[datas[i]];
	}
